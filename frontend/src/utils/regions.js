 ...
 * @returns {Array} 级联选择器的值数组
 */
export function getRegionValue(province, city, district) {
  if (!province) return []
  
  const result = []
  
  const provinceItem = regionData.find(p => p.value === province || p.label === province)
  if (provinceItem) {
    result.push(provinceItem.value)
    
    if (city && provinceItem.children) {
      const cityItem = provinceItem.children.find(c => c.value === city || c.label === city)
      if (cityItem) {
        result.push(cityItem.value)
        
        if (district && cityItem.children) {
          const districtItem = cityItem.children.find(d => d.value === district || d.label === district)
          if (districtItem) {
...
/**
 * 从级联选择器值解析出省市区名称
 * @param {Array} value - 级联选择器的值数组
 * @returns {Object} 包含 province, city, district 的对象
 */
export function parseRegionValue(value) {
  if (!value || value.length === 0) {
    return { province: null, city: null, district: null }
  }
  
  return {
    province: value[0] || null,
    city: value[1] || null,
    district: value[2] || null
  }
}
